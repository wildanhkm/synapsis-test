...
const EditPost: FC = (props: Props) => {
  ...
  const savePost = async (id: string | string[]) => {
    try {
      const { status } = await axios.put(`/public/v2/posts/${id}`);
  ...
  return (
    <main className="bg-white dark:bg-black flex min-h-screen flex-col gap-4 px-36 py-12 font-inter">
      <h1 className="text-black dark:text-white font-bold text-xl text-center">Edit Blog Post</h1>
      <input
      ...
      <div className="flex gap-4">
        <Link
          href={`/blog/${id}`}
          className="bg-white dark:bg-black text-black dark:text-white rounded border p-2 hover:bg-slate-100 dark:hover:bg-slate-800"
        >
          Cancel
        </Link>
